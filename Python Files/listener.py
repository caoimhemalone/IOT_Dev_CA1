...
myAWSIoTMQTTClient.subscribe("iotdevca1thing", 1, customCallback)
time.sleep(2)
...
def Camera_Method():
  while cam_state:
      try:
              ...
              digitalWrite(buzzer, 0)
              result = myAWSIoTMQTTClient.publish('raspberryPI', {"camera": 1})
              print result
              time.sleep(5)
...
def Green_Method():
  while led2_state:
      try:
          ...
          digitalWrite(led2sensor, 1)
          time.sleep(1)
          result = myAWSIoTMQTTClient.publish('raspberryPI', "Green LED On", 1)
          print result
            

          digitalWrite(led2sensor, 0) 
          time.sleep(1)
          result = myAWSIoTMQTTClient.publish('raspberryPI', "Green LED Off", 0)
          print result
...
def Red_Method():
  while led3_state:
      try:
          ...
          time.sleep(1)
          result = myAWSIoTMQTTClient.publish('raspberryPI', "Red LED On", 1)
          print result
           

          digitalWrite(led3sensor, 0)
          result = myAWSIoTMQTTClient.publish('raspberryPI', "Red LED Off", 0)
          print result
